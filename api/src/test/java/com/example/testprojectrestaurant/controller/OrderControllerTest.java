...
class OrderControllerTest {
    ...
    void shouldAddNewOrder() {
        ...
        Mockito.when(orderMapper.toDto(order)).thenReturn(new OrderResponseDto(order.getId(),
                order.getProduct(), order.getFunction().name(), order.getUser().getId()));
        Mockito.when(orderService.save(order)).thenReturn(new Order(order.getId(),
                order.getProduct(), order.getFunction(), order.getUser()));
        ...
        RestAssuredMockMvc.given()
                ...
                .body("function", Matchers.equalTo("RECEIVED"))
    ...
    void shouldUpdateOrderStatus() {
        OrderUpdateDto dto = new OrderUpdateDto("Product", Order.Function.ACCEPT.name(), 1L);
        Mockito.when(orderMapper.updateToModel(dto)).thenReturn(order);

        Mockito.when(orderMapper.toDto(order)).thenReturn(new OrderResponseDto(order.getId(),
                order.getProduct(), dto.function(), order.getUser().getId()));

        Mockito.when(orderService.update(order)).thenReturn(new Order(order.getId(),
                order.getProduct(), Order.Function.valueOf(dto.function()), order.getUser()));
        ...
        RestAssuredMockMvc.given()
                ...
                .body("id", Matchers.equalTo(1))
                .body("product", Matchers.equalTo("Product"))
                .body("function", Matchers.equalTo("ACCEPT"))
                .body("userId", Matchers.equalTo(1));
